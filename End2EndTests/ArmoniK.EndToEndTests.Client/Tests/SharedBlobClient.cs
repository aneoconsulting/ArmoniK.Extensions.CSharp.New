  ...
  public class SharedBlobClient : ClientBase
  {
    ...
    public async Task SharedBlob()
    {
      ...
      await Client!.EventsService.WaitForBlobsAsync(SessionHandle!,
                                                    allResults.Select(blobHandle => blobHandle!.BlobInfo).AsICollection(),
                                                    CancellationToken.None)
                   .ConfigureAwait(false);

      string referenceInputBlobId = inputBlobDefinition.BlobHandle!.BlobInfo.BlobId;
      foreach (var blobHandle in allResults)
      {
        var inputBlobId = Encoding.UTF8.GetString(await blobHandle!.DownloadBlobDataAsync(CancellationToken.None).ConfigureAwait(false));
